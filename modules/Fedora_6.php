...
$os_config = array(
	         "os_name" => "Fedora",
	         "os_vendor" => "Fedora",
                   "os_version" => "6", // use a _ instead of . for a "dot-version" delimiter (this is for compatibility with MySQL and class constructor naming conventions
                   "os_subversion" => "",
                   "os_friendly_name" => "Fedora Core 6",
                   "os_mirror" => "http://mirrors.kernel.org/fedora/core/6/i386/os", // a remote Internet based mirror (used instead of local mirror if os_mirror_local is 0
...
class OS_Fedora_6_Deploy extends HTML_QuickForm { // Note the formatting (strip the "dot" in the version out)              
	function OS_Fedora_6_Deploy($op) { 
	...
	function writeFile() {
		...
		*/ 	
		...
		if (OS_MIRROR_LOCAL) { // check the "local" flag to determine if the OS mirror is local or installed from the Internet or some other source
			$mirror = new File_SearchReplace("{{mirror}}",MIRROR_SERVER ."/". OS_NAME ."/". OS_VERSION ."/". $_POST['cpu'] ."/os", CONFIGFILES ."". $_POST['mac']); 
			$mirror->doSearch();
		}
		else { 
			$mirror = new File_SearchReplace("{{mirror}}",OS_MIRROR ."/". $_POST['cpu'] ."/os", CONFIGFILES ."". $_POST['mac']); 
			$mirror->doSearch();	
		}
	...
	function generate_pxe_boot_config() { // maybe this can be generic enough to move to the FastDeployClasses?
        ...
        append ksdevice=eth0 load_ramdisk=1 vnc vncconnect=10.187.49.100 headless prompt_ramdisk=0 ramdisk_size=16384 initrd=installer/". $_POST['cpu'] ."/". OS_NAME ."/". OS_VERSION ."/initrd.img network ks=". CONFIGFILES_URL ."". $_POST['mac'];
