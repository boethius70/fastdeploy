...
$os_config = array(
	         ...
	         "os_vendor" => "Microsoft",
                   "os_version" => "XP",
                   "os_friendly_name" => "Windows XP Professional", 
                   "os_subversion" => "Professional",
...
class OS_Windows_XP_Professional_Deploy extends HTML_QuickForm { 
	/* display section : actually regurgitate information to the user */ 
	
	function OS_Windows_XP_Professional_Deploy($op) { 
	...
	function setForm() { 
		...
		$fields = array("HOSTNAME" => array($form_strings['LBL_HOSTNAME'],"TEXT","hostname","required",array("size" => "25")),
		      ...
		      "MAC"=> array($form_strings["LBL_MAC"],"TEXT","mac","required",array("size" => "25", "onkeyup" => "chkThis(this);")),
		      ...
		      "PRODUCTID" => array($form_strings["LBL_PRODUCTID"],"TEXT","productid","required",array("size" =>"35", "onkeyup" => "chkPK(this);")),
...
SubnetMask = ". $_REQUEST['netmask'] .""; 
				...
				if ($col == "post_install") { // we have to do this separately if the column value is blank otherwise autologon will stay unblanked-out
					if ($value == "") { // blank these values if there no post_install options chosen	
						$post_install_change = new File_SearchReplace("{{post_install}}","",CONFIGFILES ."/". $short_mac .".txt"); 
						$post_install_change->doSearch();	
						$autologon_change = new File_SearchReplace("{{autologon}}","Off",CONFIGFILES ."/". $short_mac .".txt"); 
						$autologon_change->doSearch();		
					}
				}
...
parted /dev/". $harddrive[$_POST['hd']] ." mklabel msdos			
parted /dev/". $harddrive[$_POST['hd']] ." mkpart primary fat32 0 100%
parted /dev/". $harddrive[$_POST['hd']] ." set 1 boot on
...
echo \'Rebooting now...\'
reboot
"; // this final carriage return matters // this final carriage return matters
