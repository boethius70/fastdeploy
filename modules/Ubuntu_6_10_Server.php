...
$os_config = array(
	         "os_name" => "Ubuntu",
	         "os_vendor" => "Ubuntu",
                   "os_version" => "6_10", // use a _ instead of . for a "dot-version" delimiter (this is for compatibility with MySQL and class constructor naming conventions
                   "os_subversion" => "Server",
                   "os_friendly_name" => "Ubuntu 6.10 Server",   
                   "os_mirror" => "http://mirrors.kernel.org/ubuntu", // a remote Internet based mirror (used instead of local mirror if os_mirror_local is 0
                   "os_mirror_local" => 1, // toggle local or remote installation mirrors (0 = remote, 1 = local)
                   "os_icon" => "ubuntu_icon.png",
...
class OS_Ubuntu_6_10_Server_Deploy extends HTML_QuickForm { // Note the formatting (strip the "dot" in the version out)              
	function OS_Ubuntu_6_10_Server_Deploy($op) { 
	...
	function errorCheck() { 
		...
		*/
		...
		foreach ($_POST as $error_column => $error_value) { 
			...
			if ($error_value == "") { 	
				array_push ($errors,"<font color=red>". OS_Ubuntu_6_10_Server_Deploy::errorForm($error_column) ." </font><BR>"); 	
			}	
		}
		return $errors; 	
	}
	...
	function writeFile() {
		...
		*/ 	
		...
		$mirror_server = new File_SearchReplace("{{mirror_server}}",$base_mirror_server ."/". OS_NAME ."/". OS_VERSION ."/". OS_SUBVERSION, CONFIGFILES ."". $short_mac); // we're doing it this way because debian preseed doesn't need a full mirror URI
	...
	function generate_pxe_boot_config() { // maybe this can be generic enough to move to the FastDeployClasses?
        ...
        kernel ub/linux
        append initrd=ub/initrd.gz ramdisk_size=16384 root=/dev/rd/0 rw debian-installer/locale=en_US kbd-chooser/method=\"American English\" netcfg/choose_interface=eth0 console-setup/layoutcode=us DEBCONF_DEBUG=5 netcfg/get_hostname=". $_POST['hostname'] ." preseed/url=". CONFIGFILES_URL ."/". $short_mac ." --";
