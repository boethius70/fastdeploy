...
$os_config = array(
	         "os_name" => "CentOS",
	         "os_vendor" => "CentOS",
                   "os_version" => "4_5", // use a _ instead of . for a "dot-version" delimiter (this is for compatibility with MySQL and class constructor naming conventions
                   "os_subversion" => "",
                   "os_friendly_name" => "CentOS 4.5",
                   "os_mirror" => "http://mirrors.kernel.org/centos/4/os/i386", // a remote Internet based mirror (used instead of local mirror if os_mirror_local is 0
...
class OS_CentOS_4_5_Deploy extends HTML_QuickForm { // Note the formatting (strip the "dot" in the version out)              
	function OS_CentOS_4_5_Deploy($op) { 
	...
	function generate_pxe_boot_config() { // maybe this can be generic enough to move to the FastDeployClasses?
        ...
        append ksdevice=eth0 vnc load_ramdisk=1 prompt_ramdisk=0 ramdisk_size=16384 initrd=installer/". $_POST['cpu'] ."/". OS_NAME ."/". OS_VERSION ."/initrd.img network ks=". CONFIGFILES_URL ."". $_POST['mac'];
