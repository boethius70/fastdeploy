...
// Purpose of this script:  This is a plugin script for Windows 2003 Server
// Automated var (unattended) installs. 

$os_config = array(
	         "os_name" => "Debian",
	         "os_vendor" => "Debian",
                   "os_version" => "3_1", // use a _ instead of . for a "dot-version" delimiter (this is for compatibility with MySQL and class constructor naming conventions
                   "os_subversion" => "",
                   "os_friendly_name" => "Debian 3.1",   
                   "os_mirror" => "http://mirrors.kernel.org/debian", // a remote Internet based mirror (used instead of local mirror if os_mirror_local is 0
                   "os_mirror_local" => 1, // toggle local or remote installation mirrors (0 = remote, 1 = local)
                   "os_icon" => "debian_icon.png",
...
class OS_Debian_3_1_Deploy extends HTML_QuickForm { // Note the formatting (strip the "dot" in the version out)              
	function OS_Debian_3_1_Deploy($op) { 
	...
	function errorForm($val) { 
		...
		$fields = array("HOSTNAME" => array($error_strings['ERR_HOSTNAME'],"hostname"),
		      ...
		      "REDEPLOY" => array($error_strings["ERR_REDEPLOY"],"redeploy"),
	...
	function errorCheck() { 
		...
		*/
		...
		foreach ($_POST as $error_column => $error_value) { 
			...
			if ($error_value == "") { 	
				array_push ($errors,"<font color=red>". OS_Debian_3_1_Deploy::errorForm($error_column) ." </font><BR>"); 	
			}	
		}
		return $errors; 	
	}
	...
	function writeFile() {
		...
		*/ 	
		...
		$short_mac = shortMAC($_POST['mac']); 
		system("cp ". TEMPLATES ."". OPSYS .".cfg ". CONFIGFILES ."/". $short_mac ."");
		...
		foreach ($_POST as $col => $value) { 
			...
			if ($value != "") {
				...
				elseif ($col == "software") { 
					...
					if ($value != "") { 
						for ($x=0; $x<sizeof($value); $x++) { // we have to pull field values from EACH potential profile and write them out
								...
								$values .= $check_prof['script_text'] .","; // write the script_text to the file...
						}
						$values=rtrim($values,","); 
						$value=$values; // flatten the returned profiles now...
					//$value=implode("\n",$value); // flatten the profiles array so it can be written and formatted to the file properly
					}
				}
				elseif ($col == "post_install") { // this is done to append the ops.php script call to the post_install area
					if ($_POST['notify'] == 'yes') { 
						$ops_notify = "wget -q \"http://". BASEURL ."/ops.php?op=1&sessid=". $check_server['sessid'] ."&serverid=". $check_server['serverid'] ."&customer_id=". $check_server['customer_id'] ."&os_name=". OS_NAME ."&os_version=". OS_VERSION ."\"";
					} 
					...
					$value = $value ."; ". $ops_notify ."; ". $delete_files;	
				}
				elseif ($col == "addr_type") {
					if ($value == "static") {
						$static_change = new File_SearchReplace("{{static_flag}}","true", CONFIGFILES ."".$short_mac); // enable static IP addressing
						$static_change->doSearch();	
						$dhcp_change = new File_SearchReplace("{{dhcp}}","true", CONFIGFILES ."".$short_mac); // turn off dhcp support...
						$dhcp_change->doSearch();	
					}
					else { 
						$static_change = new File_SearchReplace("{{static_flag}}","false", CONFIGFILES ."".$short_mac); // disable static IP addressing
						$static_change->doSearch();	
						$dhcp_change = new File_SearchReplace("{{dhcp}}","false", CONFIGFILES ."".$short_mac); // turn on dhcp support...
						$dhcp_change->doSearch();	
					}	
					$value = $value; 
				}
			
				$snr = new File_SearchReplace("{{". $col ."}}",$value, CONFIGFILES ."".$short_mac); 
				$snr->doSearch();
			}
			else {
				if ($col == "post_install") { // this is done to append the ops.php script call to the post_install area
					...
					$delete_files = "wget -q \"http://". $_SERVER['SERVER_NAME'] ."/ops.php?op=2&sessid=". $check_server['sessid'] ."&serverid=". $check_server['serverid'] ."&customer_id=". $check_server['customer_id'] ."&os_name=". OS_NAME ."&os_version=". OS_VERSION ."\"";
					$value = $ops_notify ."; ". $delete_files;	
				}
				$blankit = new File_SearchReplace("{{". $col ."}}","". $value ."", CONFIGFILES ."". $short_mac); // just blank the template value out
		...
		if (OS_MIRROR_LOCAL) { // check the "local" flag to determine if the OS mirror is local or installed from the Internet or some other source
			$base_url = ereg_replace("http://","",BASEURL); // don't need the URL for the preseed file...		
			$mirror = new File_SearchReplace("{{mirror}}",$base_url, CONFIGFILES ."". $short_mac); // we're doing it this way because debian preseed doesn't need a full mirror URI
			$mirror->doSearch();
			$base_mirror_server = ereg_replace("http://". $base_url, "", MIRROR_SERVER); // this also accomodates the preseed file format
			$mirror_server = new File_SearchReplace("{{mirror_server}}",$base_mirror_server ."/". OS_NAME ."", CONFIGFILES ."". $short_mac); // we're doing it this way because debian preseed doesn't need a full mirror URI
			$mirror_server->doSearch();
		}
		else { 
			$base_url = ereg_replace("http://","",OS_MIRROR); // don't need the URL for the preseed file...		
			$mirror = new File_SearchReplace("{{mirror}}",$base_url, CONFIGFILES ."". $short_mac); // we're doing it this way because debian preseed doesn't need a full mirror URI
			$mirror->doSearch();
			$base_mirror_server = ereg_replace("http://". $base_url, "", OS_MIRROR); // this also accomodates the preseed file format
			$mirror_server = new File_SearchReplace("{{mirror_server}}",$base_mirror_server ."/debian", CONFIGFILES ."". $short_mac); // we're doing it this way because debian preseed doesn't need a full mirror URI
			$mirror_server->doSearch();
		}
		
		// write to tftpboot area now...
		system ("cp ". BOOTCONFIG ."/template ". BOOTCONFIG ."/01-". $_POST['mac'] .""); // note the preceding "01-" - this is required by pxelinux 
	...
	function generate_pxe_boot_config() { // maybe this can be generic enough to move to the FastDeployClasses?
        ...
        kernel installer/". $_POST['cpu'] ."/". OS_NAME ."/". OS_VERSION ."/linux
        append initrd=26/initrd.gz ramdisk_size=16384 root=/dev/rd/0 rw languagechooser/language-name=English netcfg/choose_interface=eth0 countrychooser/shortlist=US DEBCONF_PRIORITY=critical console-keymaps-at/keymap=us preseed/url=". CONFIGFILES_URL ."/". $short_mac ." --";
	...
	function timezones($tzone) {
 	...
 	function architecture ($cpu) {
   	...
   	*/
   		$cpu = array("" => "-->Pick a CPU type", // note the architecture's nomenclature here; this is referenced by 
